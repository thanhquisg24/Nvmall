...
$(function(){
	...
	function get_info_category_product(){
		...
		extras_GET_json('CategoryController','get_category_product_by_id',pdata,function(data){
			if(data!=null)
			{
	...
	function load_property(type_id){
		var pdata = {
				'type_id':type_id
		};
		extras_GET_json('CategoryController','get_property_by_product_type',pdata,function(data){
			if(data!=null)
			{
				var strhtml = '';
				//brand
				...
				//category
				var list_cate = data.list_category;
				if(list_cate.length > 0){
					var strhtml_a_ca = '<div class="box_filter clear_fix col-xs-12">';
					strhtml_a_ca+='<div class="col-xs-2 bf_left">Danh mục</div>';
					strhtml_a_ca+='<div class="col-xs-10 bf_attr">';
					strhtml_a_ca+='<div class="list_attr list_brand_cate">@content_brand</div>';
					strhtml_a_ca+='</div></div>';
					
					var strhtml_ca = '';
					for(var i=0;i<list_cate.length;i++){
						var item_cate = '<div class="item-property"><input type="checkbox" class="item_category"  name="filter_atrr" id="ft_cate_@id"/>';
						item_cate +='<label   for="ft_cate_@id"><span>@name';					
						item_cate +='</span></label></div>';
						item_cate = item_cate.replace(/\@id/g,list_cate[i].product_type_id);					
						item_cate = item_cate.replace(/\@name/g,list_cate[i].product_type_name);
						strhtml_ca+=item_cate;
					}
					strhtml_a_ca = strhtml_a_ca.replace('@content_brand',strhtml_ca);				
					strhtml+=strhtml_a_ca;
				}
	...
	function get_item_menu(callback){
		var url = ReturnHosing_apache()+'txt/menu_search_category_product.txt';
