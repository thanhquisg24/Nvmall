  ...
  if (!("onhashchange" in window)) {
    ...
    window.onhashchange = function(){
      if(!that.linkClicked){
    		...
    		var array_tag=taglocation.split("?");
    		
    		// console.log("getHashtag: "+ array_tag[0]);
        that.currentState = array_tag[0];
        
        console.log("Via.prototype.watchHashChange currentState : "+ that.currentState);
        that.loadPage();
      }
      else{
        that.loadPage();
