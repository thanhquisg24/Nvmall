$(function(){
	...
	function load_datagrid_sub(parrentid){
		$('#iddatagrip').datagrid({
			toolbar: '#tbar',
		    url: extras_Hosting["tomcatSpring_context"]+'ad/ad_Eproductypesub/get_json_append_to_datagrip.json?parentid='+parrentid,
		    idField:'id',
		    remoteSort:false,
		    singleSelect:true,
		    columns:[[
	...
	function create_panel(id,tabtitle){
		$.get(extras_Hosting["apache"] +"admin/views/E_producttypesub/producttypesub_edit.html",function(data){
			if(data!=null){
	...
	$("#ideditfunction").click(function(){
		...
		extras_GET_json(true,"ad/ad_Eproductypesub","get_prodcutypesubfull_by_id",{'id':id},function(data){
    				...
    				$('#dialog_function').data('sgrid','#iddatagrip');
